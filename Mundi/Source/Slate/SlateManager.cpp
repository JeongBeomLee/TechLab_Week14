...
void USlateManager::OpenAssetViewer(UEditorAssetPreviewContext* Context)
{
    ...
    for (SWindow* Window : AllWindows)
    {
        ...
        if (ViewerType == EViewerType::Skeletal && dynamic_cast<SSkeletalMeshViewerWindow*>(Window))
            ...
            bIsSameType = true;
        else if (ViewerType == EViewerType::PhysicsAsset && dynamic_cast<SPhysicsAssetEditorWindow*>(Window))
            bIsSameType = true;
        else if (ViewerType == EViewerType::PhysicalMaterial && dynamic_cast<SPhysicalMaterialEditorWindow*>(Window))
            bIsSameType = true;
